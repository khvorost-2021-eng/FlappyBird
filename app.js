    ...
    async getLeaderboard() {
        ...
        try {
            ...
            return r.entries.map(e => ({
                rank: e.rank,
                name: e.player.publicName || T.guest,
                score: e.score,
                avatar: e.player.getPhoto?.('small') || '',
...
class FlappyGame {
    ...
    addPipe() {
        ...
        if (DeviceInfo.isMobile) {
            ...
            min = -60;  // Верхняя труба уходит за край на 30px
        ...
        try {
            ...
            if (height > 60 && !DeviceInfo.isLowEnd) {
                ...
                for (let i = 0; i < msteps; i++) {
                    const my = y + i * 40 + 10;
                    ctx.beginPath();
                    ctx.arc(x + ms + 4, my, 5, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.beginPath();
                    ctx.arc(x + ms + 6, my + 10, 4, 0, Math.PI * 2);
                    ctx.fill();
                }
                ctx.globalAlpha = 1;
            }
    ...
    drawParrot(t) {
        ...
        ctx.beginPath();
        ctx.arc(0, w, 15, 0, Math.PI * 2);
        ctx.fill();
        const sp = Math.sin(t * 4) * 6 + 14;
        ctx.fillStyle = '#FF1493';
        ctx.beginPath();
        ctx.ellipse(-8, w, sp, 9, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#4169E1';
        ctx.beginPath();
        ctx.arc(8, w - 7, 8, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = 'white';
        ctx.beginPath();
        ctx.arc(11, w - 9, 3, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = 'black';
        ctx.beginPath();
        ctx.arc(12, w - 9, 1.5, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#333';
        ctx.beginPath();
        ctx.moveTo(14, w - 7);
    ...
    drawGhost(t) {
        ...
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(-5, w - 2, 3, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(5, w - 2, 3, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(0, w + 4, 3, 0, Math.PI * 2);
        ctx.fill();
    }
