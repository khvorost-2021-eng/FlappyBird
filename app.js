...
const THEMES = {
    day: {
        ...
        appBgTop: '#87CEEB', appBgMid: '#B0E0E6', appBgBottom: '#E0F6FF'
    },
    night: {
        ...
        appBgTop: '#0B1026', appBgMid: '#1A237E', appBgBottom: '#2A3580'
    },
    sunset: {
        ...
        appBgTop: '#FF6B9D', appBgMid: '#FFA751', appBgBottom: '#FFD080'
    },
    forest: {
        ...
        sunColor: '#FFEB3B', moonColor: '#FFFACD',
        appBgTop: '#A8D5BA', appBgMid: '#D4E8C2', appBgBottom: '#E8F5D8'
    }
};
...
const ScreenManager = {
    ...
    goTo(targetId, game = null) {
        ...
        // 🔧 ИСПРАВЛЕНО: если идём в меню — обновляем превью ДО начала анимации
        // чтобы когда меню станет видимым, оно уже было с актуальными данными
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                if (targetId === 'skinsScreen' && game) {
                    ...
                    // Ещё раз убеждаемся что превью актуально
                    game.updateMenuPreview();
                }
            }, 280);
        }, 280);
...
class FlappyGame {
    constructor() {
        ...
        this.pipeGap = 165;           // px — ПРЕЖНЕЕ значение (как в оригинале)
        this.pipeWidth = 60;          // px
        this.pipeSpeed = 350;         // px/s — увеличенная скорость
    ...
    handleVisibilityHidden() {
        this._wasRunning = this.gameRunning;
        this._wasStarted = this.gameStarted;
        if (this.gameRunning) {
            this.gameRunning = false;
            if (this.animationId) {
    ...
    resumeAfterRotation() { if (this.pausedForRotation) this.pausedForRotation = false; }
    ...
    renderSettingsScreen() {
        ...
        Object.entries(THEMES).forEach(([k, theme]) => {
            ...
            cvs.width = 200; cvs.height = 160; card.appendChild(cvs);
            const lbl = document.createElement('div');
            lbl.className = 'theme-label'; lbl.textContent = theme.name; card.appendChild(lbl);
            const chk = document.createElement('div');
            chk.className = 'theme-check'; chk.textContent = '✓'; card.appendChild(chk);
    ...
    async selectTheme(k) {
        ...
        gameState.theme = k;
        applyThemeToApp();
        // 🔧 НЕМЕДЛЕННО обновляем превью меню (в фоне, пока ещё на экране настроек)
    ...
    startMenuPreview() {
        ...
        let frame = 0;
        const render = () => {
            // 🔧 Превью работает даже когда меню неактивно (чтобы к моменту появления
            // оно уже было нарисовано с актуальными данными темы и скина)
    ...
    // 🔧 НОВЫЙ МЕТОД: немедленный перезапуск превью меню
    // Используется при смене темы/скина, чтобы к моменту перехода в меню
    // превью уже было с актуальными данными
    updateMenuPreview() {
        applyThemeToApp();
        this.startMenuPreview();
    }
    ...
    async buySkin(k) {
        ...
        // 🔧 НЕМЕДЛЕННО обновляем превью меню (чтобы при возврате оно уже было с новым скином)
        this.updateMenuPreview();
        await YandexAPI.savePlayerData(gameState);
        applyThemeToApp();
        this.updateAllCoinDisplays();
    ...
    async selectSkin(k) {
        gameState.currentSkin = k;
        // 🔧 НЕМЕДЛЕННО обновляем превью меню
        this.updateMenuPreview();
        await YandexAPI.savePlayerData(gameState);
        this.renderSkinsGrid();
    }
    ...
    resetGame() {
        ...
        this.earnedCoins = 0;
        this.pipesPassed = 0;
        this.pipeTimer = this.pipeInterval - 0.5;
        this.frameCount = 0;
        this.gameTime = 0;
    ...
    render() {
        ...
        if (theme.hasStars) {
            ...
            for (const star of this.stars) {
                const tw = Math.sin(this.gameTime * star.speed + star.twinkle) * 0.5 + 0.5;
                ctx.globalAlpha = tw;
    ...
    drawClouds(theme) {
        ...
        for (const cloud of this.clouds) {
            const dist = cloud.speed * this.gameTime;
            const x = ((cloud.baseX - dist) % totalW + totalW) % totalW + this.visibleLeft - 100;
            const y = cloud.y;
            const s = cloud.scale;
            ctx.beginPath();
    ...
    drawCoin(coin) {
        ...
        const sc = Math.abs(Math.cos(this.gameTime * 6));
    ...
    drawBird() {
        ...
        let angle = 0;
        if (this.gameStarted) angle = Math.max(-0.5, Math.min(0.8, this.bird.velocity / 600));
        ctx.rotate(angle);
        try { this.drawSkin(this.gameTime * 3, gameState.currentSkin); }
        catch (e) { this.drawStandardBird(this.gameTime * 3); }
        ctx.restore();
    }
